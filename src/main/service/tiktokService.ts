...
function applyMapping(src: any, mapping: Record<string, string>) {
    const out: any = {};
    for (const [from, to] of Object.entries(mapping || {})) out[to] = getByPath(src, from);
    return out;
}
...
export class TikTokService {
    ...
    private readonly skipVerify =
        process.env.SKIP_SIGNATURE_VERIFY === '1' || process.env.NODE_ENV !== 'production';
    ...
    async processWebhook(
        ...
        if (!this.skipVerify) {
            ...
            const bodyBuff: Buffer =
                rawBody ?? Buffer.from(typeof payload === 'string' ? payload : JSON.stringify(payload));
        ...
        const phone = normalizePhone(payload?.lead_data?.phone ?? undefined) ?? undefined;
