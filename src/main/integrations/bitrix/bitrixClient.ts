...
export class BitrixClient {
    ...
    async createLead(fields: Record<string, any>): Promise<number> {
        return this.call<number>('crm.lead.add', { fields, params: { REGISTER_SONET_EVENT: 'N' } });
    }
    ...
    async createDeal(fields: Record<string, any>): Promise<number> {
        return this.call<number>('crm.deal.add', { fields, params: { REGISTER_SONET_EVENT: 'N' } });
    }

    async createContact(fields: Record<string, any>): Promise<number> {
        return this.call<number>('crm.contact.add', { fields, params: { REGISTER_SONET_EVENT: 'N' } });
    }

    // Core call helper
    private async call<T>(method: string, body: any = {}, config?: AxiosRequestConfig): Promise<T> {
